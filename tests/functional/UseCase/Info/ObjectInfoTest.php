...
class ObjectInfoTest extends BaseCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->createTestTable(
            ...
            $bucketResponse->getCreateBucketObjectName(),
            'bucket_table1'
        );
        $db = $this->sessionManager->createSession($this->projectCredentials);
        $db->executeStatement(sprintf(
            'CREATE VIEW %s."bucket_view1" AS '
            . 'SELECT * FROM %s."bucket_table1";',
            TeradataQuote::quoteSingleIdentifier($bucketResponse->getCreateBucketObjectName()),
            TeradataQuote::quoteSingleIdentifier($bucketResponse->getCreateBucketObjectName())
        ));
        // create workspace
        [
    ...
    private function assertDatabase(ObjectInfoResponse $response, SchemaInfo|DatabaseInfo $infoObject): void
    {
        ...
        $workspaceObject = $this->getObjectByNameAndType(
            $objects,
            $this->workspaceResponse->getWorkspaceUserName()
        );
